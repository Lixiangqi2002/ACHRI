...
model = LateFusionModel(ppg_encoder, hr_encoder, thermal_encoder, fusion_module)
...
# Training...
############
# Define a custom dataset for your multimodal data.
class EmotionDataset(Dataset):
    def __init__(self, ppg_data, thermal_data, hr_data, labels):
        """
        ppg_data: numpy array of shape (N, seq_length_ppg, 2)
        thermal_data: numpy array of shape (N, seq_length_thermal, 1)
        hr_data: numpy array of shape (N, seq_length_hr, 1)
        labels: numpy array of shape (N,) with emotion scores in [0, 9]
        """
    ...
    def __getitem__(self, idx):
        return (torch.tensor(self.ppg_data[idx], dtype=torch.float32),
                torch.tensor(self.thermal_data[idx], dtype=torch.float32),
                torch.tensor(self.hr_data[idx], dtype=torch.float32),
...
# Set up your model.
# For example, if you have a LateFusionModel defined (which uses your pre-trained encoders and a fusion module):
device = torch.device("mps" if torch.backends.mps.is_available() else "cpu")
model.to(device)
print("Using device:", device)
...
plt.plot([0, 9], [0, 9], color='red', linestyle='--')  # Perfect prediction line
plt.xlim([0, 9])
plt.ylim([0, 9])
